...
## A grande novidade da 1.0.6 / 0.14.4

### Inicialização Fabric e bytecode JVM realmente corrigidos

- Corrigido o `VerifyError: Illegal type at constant pool entry 66` encontrado no teste da 1.0.5.
- O acesso ao Fabric Loader foi isolado em `FabricLoaderAccess`, compilado contra a interface real.
- As cinco chamadas consumidoras usam métodos estáticos do boundary, preservando offsets e frames.
- O verificador agora cruza `invokevirtual`, `invokestatic` e `invokeinterface` com o tipo exato da constante.
- Uma troca incompleta de `Methodref` por `InterfaceMethodref`, causa da regressão 1.0.5, bloqueia a release.
- O teste cobre todas as classes Lyrius e não depende de esperar o jogo crashar.
- O antigo problema de registro duplicado do canal `lyriusrp:json_payload` continua corrigido.

### Atualizador conectado ao GitHub

- O plugin 0.14.4 já traz o endereço do manifesto oficial em sua configuração padrão.
...
2. Coloque `LyriusRpModPlugin-1.0.6.jar` na pasta `mods` do cliente.
3. Coloque `LyriusReputation-0.14.4.jar` na pasta `plugins` do Paper.
